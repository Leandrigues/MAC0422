...
{
    pid_t process;
    int priority, pidCaller;
    int exit;
    int rmp_nr;
    struct mproc* rmp;
    struct mproc*  parentProcess;
    
    pidCaller = m_in.m1_i2;
    process = m_in.m1_i1;

    priority = 15;
    rmp_nr = proc_from_pid(process);
    rmp = &mproc[rmp_nr];
    parentProcess = &mproc[rmp->mp_parent];
    
    if (parentProcess->mp_pid != pidCaller) {  
        printf("Erro, batch() deve ser chamada pelo processo pai\n");
	return -1;
    }
    
    printf("Chamando sys_insert para %d com prioridade %d\n", process, priority);
    exit = sys_insert(rmp->mp_endpoint, priority);
